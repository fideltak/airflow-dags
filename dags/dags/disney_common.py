...
def make_task(task_id, arguments, memory="2Gi", cpu="1",
    ...
    # ★S3 の認証を env としても渡す
    #   1) 手動キー Secret があればそれを優先
    #   2) 無ければ PCAI の access-token Secret を
    #      AWS_ACCESS_KEY_ID=<AUTH_TOKEN>, AWS_SECRET_ACCESS_KEY=s3
    #      に写して渡す
    #
    #   後者は HPE 公式の S3 プロキシ利用方法そのもの。
    #   こうしておくと、古い disney-jobs イメージ(= token file を
    #   まだ自動検出しない版)でもそのまま動く。
    if S3_SECRET:
    ...
    elif EZUA_TOKEN_SECRET:
        env_vars.append(k8s.V1EnvVar(
            name="AWS_ACCESS_KEY_ID",
            value_from=k8s.V1EnvVarSource(
                secret_key_ref=k8s.V1SecretKeySelector(
                    name=EZUA_TOKEN_SECRET,
                    key="AUTH_TOKEN",
                    optional=True)),
        ))
        env_vars.append(k8s.V1EnvVar(
