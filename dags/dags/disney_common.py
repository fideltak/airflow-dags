...
def _overridable(var_name, value):
    ...
    ★Variable が無いときにログへ ERROR を出さない
      Airflow 3 の `var.value.get(key, default)` は、キーが無いと
      「Variable not found ... status_code=404」を ERROR として記録する。
      既定値で動くのが正常な使い方なのに、毎回エラーが出ていては
      本物の異常が埋もれてしまう。
      `var.value.get(...) or 既定値` ではなく Jinja の `default` を
      使い、未定義でも静かに既定値へ落ちるようにする。

    テンプレートが使えない場所(Secret 名など)では使わないこと。
    """
    safe = str(value).replace("'", "")
    # var.value は未定義キーにアクセスすると Undefined になる。
    # Jinja の default フィルタで拾えば、問い合わせの失敗を
    # ERROR として残さずに済む。
    return "{{ var.value.%s | default('%s', true) }}" % (var_name, safe)
    safe = str(value).replace("'", "")
...
def worker_selftest():
    ...
    live = {k: settings.get(k) for k in
            ("IMAGE", "S3_ENDPOINT", "S3_BUCKET", "S3_PREFIX", "S3_SECRET",
             "NAMESPACE", "TRAIN_MEMORY", "TRAIN_CPU")}
    say(f"   イメージ     : {live['IMAGE']}")
    say(f"   S3           : {live['S3_ENDPOINT']}")
    say(f"   バケット     : {live['S3_BUCKET']} / {live['S3_PREFIX']}")
    say(f"   鍵の Secret  : {live['S3_SECRET']}")
    say(f"   namespace    : {live['NAMESPACE'] or '(Airflow と同じ)'}")
    say("")

    # ★管理者が Airflow Variable で上書きしている場合は、そちらが勝つ
    #   ここはタスクの実行中なので、Variable を読んでも
    #   共有スケジューラの負担にはならない。
    try:
        from airflow.models import Variable

        overrides = {}
        for var, key in (("disney_image", "IMAGE"),
                         ("disney_s3_endpoint", "S3_ENDPOINT")):
            v = Variable.get(var, default_var=None)
            if v and v != live[key]:
                overrides[var] = v
        if overrides:
            say("   ⚠️  Airflow Variable による上書きがあります(そちらが使われます)")
