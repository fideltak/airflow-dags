...
| バケット | `{S3_BUCKET}` |
| プレフィックス | `{S3_PREFIX}` |
| Ezmeral トークン Secret | `{EZUA_TOKEN_SECRET or '(未設定)'}` |
| データ | `{DATA_DIR}` |
| 成果物 | `{MODEL_DIR}` |
...
> **認証について**: PCAI と Ezmeral Data Fabric が接続済みであれば、
> `{EZUA_TOKEN_SECRET}` Secret が自動作成されています。
> Pod にボリュームマウントすることで、手動のアクセスキー不要で S3 に接続します。
"""
...
with DAG(
    ...
    **dag_kwargs(),
    ...
    #
    # ★「起動したか」だけを見る(モデルの有無は問わない)
    #   以前は `status` を実行していたが、これはモデルが無いと
    #   終了コード 1 を返す。初回は必ずモデルが無いので、
    #   **イメージは正常に起動しているのに毎回スキップ**され、
    #   しかも後続(3_secret 以降)まで飛ばされていた。
    #   ここで確かめたいのは「イメージを取得して中の python が
    #   動くか」だけなので、必ず成功する形にする。
    image_ok = make_task(
        "2_image",
        [
            "import platform, disney, xgboost, pandas; "
            "print('OK: イメージが起動しました'); "
            "print('  python  :', platform.python_version()); "
            "print('  xgboost :', xgboost.__version__); "
            "print('  pandas  :', pandas.__version__)"
        ],
        # ★ENTRYPOINT を上書きする
        #   このイメージは `python -m disney.tasks` が入口なので、
        #   そのままだと引数がサブコマンド名として解釈されてしまう。
        cmds=["python", "-c"],
    ...
    # --- 3. S3 の鍵が使えるか -----------------------------------------
    # ★PCAI では鍵を作らなくてよい場合が多い
    #   Data Fabric が「接続済みデータストア」として登録されていれば、
    #   PCAI が Pod に利用者のトークンを配っている。
    #
    #       /etc/secrets/ezua/.auth_token
    #
    #   HPE 公式ドキュメントによると、S3 プロキシへは
    #       AWS_ACCESS_KEY_ID     = そのトークン
    #       AWS_SECRET_ACCESS_KEY = "s3"
    #   で通る。そのため、Secret を作らなくても動く。
    #   (出典: docs.ezmeral.hpe.com/unified-analytics/15/
    #          EzPresto/kserve-access-object-store.html)
    #
    #   ここでは「どちらの経路で鍵が用意できるか」を見る。
    #   値そのものは出さない(ログに残ると漏れる)。
    secret_ok = make_task(
        "3_secret",
        [
            ...
            f'  echo "OK: Secret から鍵が渡っています"; '
            '  echo "    (ID=$ID_LEN 文字 SECRET=$SEC_LEN 文字)"; '
            '  exit 0; '
            'fi; '
            'if [ -s "$TOKEN_FILE" ]; then '
            '  T_LEN=$(wc -c < "$TOKEN_FILE"); '
            '  echo "OK: PCAI が配っている認証トークンを使えます"; '
            '  echo "    $TOKEN_FILE ($T_LEN 文字)"; '
            '  echo "    → Secret を作る必要はありません"; '
            '  echo "      (S3プロキシは このトークン + \\"s3\\" で通ります)"; '
            '  exit 0; '
            'fi; '
            'echo "NG: 使える鍵がありません"; '
            'echo ""; '
            'echo "  次のどちらも見つかりませんでした:"; '
            f'echo "    1) 環境変数 AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY"; '
            f'echo "    2) PCAI の認証トークン $TOKEN_FILE (Secret: {EZUA_TOKEN_SECRET or "access-token"})"; '
            'echo ""; '
            'echo "  【確認】PCAI のデータソース接続を確認する"; '
            'echo "     Data Fabric が接続済みなら、通常このトークンは"; '
            f'echo "     Secret \'{EZUA_TOKEN_SECRET or "access-token"}\' に自動作成されます。"; '
            'echo "     無い場合は管理者にご相談ください。"; '
            'echo "     (/mnt/datasources の中身も確認してみてください)"; '
            'echo ""; '
            'echo "  --- 参考: この Pod に配られているもの ---"; '
            'ls -la /etc/secrets/ezua/ 2>/dev/null || echo "  (/etc/secrets/ezua/ なし)"; '
            'ls /mnt/datasources/ 2>/dev/null || echo "  (/mnt/datasources なし)"; '
            'exit 1'
        ],
        cmds=["sh", "-c"],
        memory="512Mi", cpu="500m",
        retries=0, timeout_minutes=5, startup_timeout_minutes=3,
        do_xcom_push=False,
        # namespace を案内文に出すために渡す
        env_vars_extra={"POD_NAMESPACE": NAMESPACE or ""},
    )
