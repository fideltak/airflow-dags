...
IMAGE = "docker.io/ryota0510/disney-jobs:2.0.3"
...
#   ★PCAI と Ezmeral Data Fabric が接続済みなら空のままで OK
#     PCAI が Pod に認証トークン(/etc/secrets/ezua/.auth_token)を配るため、
#     手動でアクセスキーを作る必要はありません。
#
#   手動で鍵を使いたい場合(接続が切れたときの代替など)は、
...
#   ★PCAI と Ezmeral が接続済みであれば、この名前の Secret が
#     ユーザーの namespace に自動作成されています。
#   Pod にボリュームとしてマウントし、
#   /etc/secrets/ezua/.auth_token として読めるようにします。
#   (storage.py が自動でそのトークンを使います)
EZUA_TOKEN_SECRET = "access-token"
